...
import { searchBusinesses, BusinessResult } from '@/lib/api/discovery';
...
import { BusinessCard } from '@/components/discovery/cards/BusinessCard';
...
function BusinessesContent() {
  ...
  useEffect(() => {
    async function fetchBusinesses() {
      setLoading(true);
      try {
        const { data, count } = await searchBusinesses({
          query: searchQuery,
          filters,
          page: currentPage,
          limit: 24,
    ...
    fetchBusinesses();
  }, [searchQuery, filters, sortBy, currentPage, businesses.length, setLoading]);
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      {businesses.length > 0 ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-500">No businesses found. Try adjusting your search or filters.</p>
        </div>
      )}
    </div>
  );
